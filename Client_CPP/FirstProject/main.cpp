// 표준입출력 스트림 헤더파일.
// stdio.h는 더이상 수동으로 추가해줄 필요 없이 자동으로 링크됨.
// C++에서는 헤더파일 확장자 없어도 됨
#include <iostream>

// namespace
// C++에서는 변수, 함수, 사용자 정의 자료형 ( 클래스 포함 ) 등을 서로 구분하기 위해
// 이름으로 사용되는 다양한 내부 식별자를 가지고 있음.
// 하지만 프로그램이 복잡해지고 여러가지 라이브러리가 포함될수록 이 식별자 간에 충돌할 가능성이 높아짐
// 이러한 이름 충돌을 방지하기 위해서 namespace를 사용함.
// 일반적으로 헤더파일에 선언해놓고 씀 .
...
int main() {
	...
	// auto 키워드
	// C 에서는 지역변수 선언 시 사용하나 생략 가능하므로 쓸모가 없음
	// C++ 에서는 해당 키워드를 재정의함.
	// auto를 자료형처럼 사용하면서 변수 선언과 동시에 초기화 하면
